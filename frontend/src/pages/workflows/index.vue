...
const fetchWorkflows = async () => {
  ...
  try {
    ...
    if (selectedProject.value) {
      const response = await axios.get(`${apiUrl}/api/v1/workflows/project/${selectedProject.value}`)
      allWorkflows = response.data
    } else {
      for (const project of projects.value) {
        try {
          const response = await axios.get(`${apiUrl}/api/v1/workflows/project/${project.id}`)
    ...
    <div class="d-flex flex-wrap justify-space-between align-center gap-4 mb-6">
      <div>
        ...
        <p class="text-body-1 mb-0">
          Gérez vos parcours utilisateurs capturés
        </p>
      </div>
    </div>
    ...
    <VCard>
      <VCardText class="d-flex align-center flex-wrap gap-4">
        ...
        <VSelect
          ...
          @update:model-value="fetchWorkflows"
        />
        <VSpacer />
